...
async.waterfall([
    ...
    function (cookies, result, callback) {
        ...
        fs.writeFile('result/gallery_list_1.txt', JSON.stringify(result.articles), (err) => {
          if (err) throw err;
            ...
            function (pageNo, next) {
                async.waterfall([
                    ...
                    function (cookies, result) {
                        fs.writeFile('result/gallery_list_' + pageNo + '.txt', JSON.stringify(result.articles), (err) => {
                        if (err) throw err;
                        console.log('The file has been saved!');
                        });
